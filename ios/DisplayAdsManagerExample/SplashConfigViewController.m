...
#import "ToastView.h"
@interface SplashConfigViewController ()<UITextFieldDelegate, UITextViewDelegate, DisplayAdsManagerDelegate>
@property (nonatomic, strong) UIScrollView *mainScrollView;
@property (nonatomic, strong) NSString *displayEndpointStr;
...
@property (nonatomic, strong) UITextView *optionalParmasTextView;
...
@implementation SplashConfigViewController

- (void)viewDidLoad {
    [super viewDidLoad];
    self.view.backgroundColor = [UIColor blackColor];
    self.title = @"Splash Config";
    self.navigationController.navigationBar.barTintColor = [UIColor blackColor];
    [self.navigationController.navigationBar setTitleTextAttributes: @{NSForegroundColorAttributeName: [UIColor whiteColor]}];
    
//    self.loadingView = [LoadingView new];
    self.toastView = [ToastView new];
    ...
    UIView *endpointContainer = [self textfiledContainerWithFrame: CGRectMake(horizontalPadding, [self getBottomPosition: endpointLabel.frame], contentWidth, 46) Placeholder: @"enter Display Endpoint" defaultText: self.displayEndpointStr tag: 1001];
    ...
    UILabel *opLabel = [self largeTitleLabelWithFrame: CGRectMake(horizontalPadding, [self getBottomPosition: slotSz.frame] + 20, contentWidth, largeLabelHeight) title: @"Optional Params"];
    [self.mainScrollView addSubview: opLabel];
    
    
    self.optionalParmasTextView = [[UITextView alloc] initWithFrame: CGRectMake(20, [self getBottomPosition: opLabel.frame] + 10, contentWidth, 150)];
    
    self.optionalParmasTextView.layer.cornerRadius = 8;
    self.optionalParmasTextView.text = self.optionalParamsStr;
    self.optionalParmasTextView.font = [UIFont systemFontOfSize: 16 weight: UIFontWeightRegular];
    self.optionalParmasTextView.backgroundColor = [UIColor colorNamed: @"TFBackgroundColor"];
    self.optionalParmasTextView.delegate = self;
    self.optionalParmasTextView.textColor = [UIColor whiteColor];
    [_mainScrollView addSubview: self.optionalParmasTextView ];
...
- (void)initDefaultValue {
    ...
    self.displayEndpointStr = [[DisplayAdsManager shareInstance] displayEndpoint];
    ...
    self.slotIuStr = @"/123456/a.pc.uat/home/splash";
    self.slotSzStr = @"320x480";
    //custParamsStr dictionary  json 字符串
    ...
    [optionalParams setObject: @"splash" forKey: @"adFormat"];
...
- (BOOL)formatVerify{
    ...
    BOOL isOK = YES;
    if ([self.displayEndpointStr isEqualToString: @""]) {
...
- (void)parseStringAndSubmit {
    ...
    [self.adManager initSpalshAdWithSlot: slot optionalParams: optionParamsNew];
...
- (void)textFieldDidEndEditing:(UITextField *)textField {
    if (textField.tag == 1001) {
        self.displayEndpointStr = textField.text;
    ...
    if (textField.tag == 1000) {
        ...
        [[DisplayAdsManager shareInstance] setDisplayEndpoint: self.displayEndpointStr  trackingEndpoint: textField.text];
    }
...
//    [self.loadingView hideLoading];
  dispatch_async(dispatch_get_main_queue(), ^{
    [self.adManager showSplash];
  });
    
}
- (void)onAdRenderFinished:(DisplayAdInstance *)adInstance {
    [self.toastView showMessage: @"Splash Render finshed"];
}
